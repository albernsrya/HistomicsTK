...
class CD_single_tissue_piece:
    ...
    def run(self):
        ...
        if self.cd.visualize_spixels:
            self.cd._print2("%s: visualize_individual_superpixels()" %
                            self.monitorPrefix)
            self.visualize_individual_superpixels()

        if self.cd.visualize_contiguous:
            self.cd._print2("%s: visualize_contiguous_superpixels()" %
                            self.monitorPrefix)
    ...
    def set_superpixel_mask(self):
        ...
        n_spixels = int(self.tissue_rgb.shape[0] * self.tissue_rgb.shape[1] /
                        spixel_size)
        ...
        if self.cd.use_grayscale:
            self.spixel_mask = slic(
                rgb2gray(self.tissue_rgb),
                n_segments=n_spixels,
                compactness=self.cd.compactness,
            )
        else:
            self.spixel_mask = slic(self.tissue_rgb,
                                    n_segments=n_spixels,
                                    compactness=self.cd.compactness)
        ...
        tmask = resize(
            self.tissue_mask,
            output_shape=self.spixel_mask.shape,
            order=0,
            preserve_range=True,
            anti_aliasing=False,
        )
    ...
    def set_superpixel_features(self):
        ...
        if self.cd.use_intensity:
            fdata_list.append(
                compute_intensity_features(im_label=self.spixel_mask,
                                           im_intensity=tissue_htx,
                                           rprops=rprops))
        if self.cd.use_texture:
            fdata_list.append(
                compute_haralick_features(im_label=self.spixel_mask,
                                          im_intensity=tissue_htx,
                                          rprops=rprops))
        ...
        self.fdata.index = set(np.unique(self.spixel_mask)) - {
            0,
        }
    ...
    def assign_cellularity_scores(self):
        ...
        for clid in np.unique(self.spixel_labels):
            self.cluster_props[clid] = {
                "cellularity":
                int(
                    np.median(self.fdata.loc[self.fdata.loc[:,
                                                            "cluster"] == clid,
    ...
    def assign_colors_to_spixel_clusters(self):
        ...
        # Assign rgb string
        for clid in np.unique(self.spixel_labels):
            cellularity = min(self.cluster_props[clid]["cellularity"],
                              max_cellularity)
    ...
    def visualize_individual_superpixels(self):
        ...
        for spval, sp in self.fdata.iterrows():
            spstr = "spixel-%d_cellularity-%d" % (
                spval,
                self.cluster_props[sp["cluster"]]["cellularity"],
            )
            GTCodes_df.loc[spstr, "group"] = spstr
            GTCodes_df.loc[spstr, "GT_code"] = spval
            GTCodes_df.loc[spstr, "color"] = self.cluster_props[
                sp["cluster"]]["color"]

        # get contours df
        contours_df = get_contours_from_mask(
            MASK=self.spixel_mask,
...
class Cellularity_detector_superpixels(Base_HTK_Class):
    ...
    def __init__(self, gc, slide_id, **kwargs):
        ...
        default_attr = {
            ...
            "cMap":
            cm.seismic,
            "visualize_tissue_boundary":
            True,
            "visualize_spixels":
            True,
            "visualize_contiguous":
            True,
        }
    ...
    def set_color_normalization_values(self,
        ...
        assert (
            all([j is not None
                 for j in (mu, sigma)]) or ref_image_path is not None
