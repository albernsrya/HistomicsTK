...
def get_scale_factor_and_appendStr(gc, slide_id, MPP=None, MAG=None):
    ...
    if (MPP is not None) and (slide_info["mm_x"] is not None):
        ...
        warnings.warn("NO SLIDE MAGNIFICATION FOUND; BASE MAGNIFICATION USED!",
                      RuntimeWarning)
        sf = 1.0
        appendStr = ""
...
def rotate_point_list(point_list, rotation, center=(0, 0)):
    ...
    for point in point_list:
        ...
        point_list_rotated.append((int(x * cos - y * sin + center[0]),
...
def get_rotated_rectangular_coords(roi_center,
                                   roi_width,
                                   roi_height,
                                   roi_rotation=0):
    ...
    roi_corners_false = [(x_min, y_min), (x_max, y_min), (x_max, y_max),
                         (x_min, y_max)]
...
def _maybe_crop_polygon(vertices, bounds_polygon):
    ...
    try:
        ...
        if polygon.geom_type in ("Polygon", "LineString"):
            ...
            polygons_to_add = [p for p in polygon if p.geom_type == "Polygon"]
    ...
    for poly in polygons_to_add:
        try:
            if polygon.area > 2:
                all_vertices.append(
                    np.array(poly.exterior.xy, dtype=np.int32).T)
        except Exception as e:
...
def _add_element_to_final_df(vertices, cfg):
    ...
    if (cfg.cropping_bounds is not None) or (cfg.cropping_polygon_vertices
                                             is not None):
    ...
    if "group" in cfg.element.keys():
        ...
        cfg.element_infos.loc[elno,
                              "group"] = str(cfg.element["label"]["value"])

    # add label or infer from group
    if "label" in cfg.element.keys():
        cfg.element_infos.loc[elno,
                              "label"] = str(cfg.element["label"]["value"])
    elif "group" in cfg.element.keys():
        cfg.element_infos.loc[elno, "label"] = cfg.element_infos.loc[elno,
                                                                     "group"]
...
def parse_slide_annotations_into_tables(
    ...
    if cfg.cropping_bounds is not None:
        assert (cfg.cropping_polygon_vertices is
                None), "either give cropping bouns or vertices, not both"
        xmin, xmax, ymin, ymax = (
            cfg.cropping_bounds["XMIN"],
        ...
        for cfg.elementidx, cfg.element in enumerate(
            ...
            if (((cfg.cropping_bounds is None) and
                 (cfg.cropping_polygon_vertices is None))
                    or (cfg.element["type"] == "point") or (not use_shapely)):
                all_coords = [
                    coords,
                ]
            else:
...
def get_idxs_for_annots_overlapping_roi_by_bbox(element_infos,
                                                idx_for_roi,
                                                iou_thresh=0.0):
    ...
    """
    bboxes = np.array(element_infos.loc[:, ["xmin", "ymin", "xmax", "ymax"]],
                      dtype="float")
...
def _get_element_mask(elinfo, slide_annotations):
    ...
    """
    element = slide_annotations[int(
        elinfo["annidx"])]["annotation"]["elements"][int(elinfo["elementidx"])]
    ...
    if element_mask.shape != patch.shape:
        element_mask = np.pad(
            ...
            pad_width=(
                ...
                (patch.shape[1] - element_mask.shape[1], 0),
            ),
            mode="constant",
        )
...
def _simple_add_element_to_roi(elinfo,
                               ROI,
                               roiinfo,
                               GT_code,
                               element=None,
