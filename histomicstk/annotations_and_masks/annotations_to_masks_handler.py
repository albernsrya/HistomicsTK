...
def get_roi_mask(
    ...
    GTCodes_df.loc[roi_group, "overlay_order"] = (
        np.min(GTCodes_df.loc[:, "overlay_order"]) - 1)
...
def get_mask_from_slide(
    ...
    bck_code = GTCodes.loc[GTCodes.loc[:, "is_background_class"] == 1,
                           "GT_code"]
...
def _visualize_annotations_on_rgb(rgb,
                                  contours_list,
                                  linewidth=0.2,
                                  x_offset=0,
                                  y_offset=0,
    ...
    fig = plt.figure(figsize=(rgb.shape[1] / 1000, rgb.shape[0] / 1000),
                     dpi=100)
    ax = plt.subplot(111)
    ax.imshow(rgb)
    ...
    for idx, ann in enumerate(contours_list):
        xy = np.array([[int(j) for j in ann[k].split(",")]
        ...
        polygon = mpPolygon(
            ...
            color=[
                int(j) / 255
                for j in ann["color"].split("rgb(")[1][:-1].split(",")
            ],
            closed=True,
            fill=False,
            linewidth=linewidth,
...
def _get_roi_bounds_by_run_mode(gc, slide_id, mode, bounds, element_infos,
                                idx_for_roi, sf):
    ...
    if mode == "polygonal_bounds":
        ...
        assert (bounds["XMAX"] > bounds["XMIN"]) and (bounds["YMAX"] >
                                                      bounds["YMIN"])
