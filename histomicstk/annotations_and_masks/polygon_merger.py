...
class Polygon_merger(Base_HTK_Class):
    ...
    def __init__(self, maskpaths, GTCodes_df, **kwargs):
        ...
        # some sanity checks
        assert not (self.contkwargs["get_roi_contour"]
    ...
    def set_contours_from_all_masks(self, monitorPrefix=""):
        ...
        for midx, maskpath in enumerate(self.maskpaths):
            ...
            for edge in ["top", "left", "bottom", "right"]:
                edgeids.extend(
                    list(contours_df.loc[contours_df.loc[:,
                                                         "touches_edge-%s" %
                                                         edge] == 1, :].index))
    ...
    def set_roi_bboxes(self, roi_offsets=None):
        ...
        for maskpath in self.maskpaths:
            ...
            with Image.open(maskpath, mode="r") as mask_obj:
                ...
                maskname = os.path.split(maskpath)[1]
                roiinfos[maskname][
                    "right"] = roiinfos[maskname]["left"] + width
                roiinfos[maskname][
                    "bottom"] = roiinfos[maskname]["top"] + height
    ...
    def set_shared_roi_edges(self):
        ...
        shared_edges = DataFrame(
            columns=["roi1-name", "roi1-edge", "roi2-name", "roi2-edge"])
        ...
        for roi_pair in roi_pairs:
            ...
            for edgepair in edgepairs:
                ...
                if (np.abs(self.roiinfos[roi1name][edgepair[0]] -
                    ...
                    realStart = np.min((self.roiinfos[roi1name][start],
                                        self.roiinfos[roi2name][start]))
                    realEnd = np.max((self.roiinfos[roi1name][end],
                                      self.roiinfos[roi2name][end]))
                    length = realEnd - realStart
                    nonoverlap_length = (self.roiinfos[roi1name][end] -
    ...
    def _get_merge_pairs(self, edgepair, group, monitorPrefix=""):
        ...
        for nid1, nest1 in Nests1.iterrows():
            ...
            try:
                ...
                coords[:, 0] = (coords[:, 0] +
                                self.roiinfos[edgepair["roi1-name"]]["left"])
                coords[:, 1] = (coords[:, 1] +
                                self.roiinfos[edgepair["roi1-name"]]["top"])
                polygons1.append((nid1, Polygon(coords)))
            except Exception as e:
                self._print2("%s: edge1-nest %d of %d: Shapely Error (below)" %
                             (monitorPrefix, nno1, nno1Max))
                self._print2(e)
        ...
        for nid2, nest2 in Nests2.iterrows():
            ...
            try:
                ...
                coords[:, 0] = (coords[:, 0] +
                                self.roiinfos[edgepair["roi2-name"]]["left"])
                coords[:, 1] = (coords[:, 1] +
                                self.roiinfos[edgepair["roi2-name"]]["top"])
                polygon2 = Polygon(coords)
            except Exception as e:
                self._print2("%s: edge2-nest %d of %d: Shapely Error (below)" %
    ...
    def _get_merge_clusters_from_df(self, merge_df, monitorPrefix=""):
        ...
        while keep_going:
            ...
            for gid, mgroup in enumerate(merge_groups["level-%d" % (level)]):
                for gid2, mgroup2 in enumerate(reference):
                    ...
                    if (gid != gid2) and (len(
                            mgroupSet.intersection(mgroup2Set)) > 0):
                        merge_groups["level-%d" % (level + 1)].append(
                            list(mgroupSet.union(set(mgroup2Set))))
                        reference[gid] = []
                        reference[gid2] = []
            # cleanup
            merge_groups["level-%d" %
                         (level)] = [j for j in reference if len(j) > 0]
        ...
        merge_clusters = [[_parse_to_dict(j) for j in cl]
    ...
    def _add_single_merged_edge_contour(self, polygon, group):
        ...
        self.merged_contours.loc[idx, "color"] = self.GTCodes_df.loc[group,
                                                                     "color"]
    ...
    def _add_merged_edge_contours(self,
                                  merged_polygons,
                                  group,
                                  monitorPrefix=""):
    ...
    def _drop_merged_edge_contours(self, merge_df):
        ...
        for roiname, edge_df in self.edge_contours.items():
            ...
            for rid in [1, 2]:
                neststr = "nest%d" % rid
                nids.extend(
                    list(merge_df.loc[merge_df.loc[:, neststr +
                                                   "-roiname"] == roiname,
                                      neststr + "-nid", ]))
    ...
    def set_merged_contours(self, monitorPrefix=""):
        ...
        for group in self.GTCodes_df.index:
            ...
            merge_df = self._get_merge_df(group=group,
                                          monitorPrefix="%s: _get_merge_df" %
                                          monitorPrefix)
    ...
    def get_concatenated_contours(self):
        ...
        for contours_dict in [self.edge_contours, self.ordinary_contours]:
            for roiname, roi_df in contours_dict.items():
                roi_df = self._add_roi_offset_to_contours(roi_df=roi_df,
                                                          roiname=roiname)
                all_contours = concat((all_contours, roi_df),
                                      axis=0,
                                      ignore_index=True)
        ...
        bottom = str(
            int(np.max([j["bottom"] for _, j in self.roiinfos.items()])))
        left = str(int(np.min([j["left"] for _, j in self.roiinfos.items()])))
        right = str(int(np.max([j["right"]
                                for _, j in self.roiinfos.items()])))
    ...
    def run(self):
        ...
        self.set_merged_contours(monitorPrefix="%s: set_merged_contours" %
                                 self.monitorPrefix)
        ...
        if self.discard_nonenclosed_background:
            ...
            all_contours = _discard_nonenclosed_background_group(
                all_contours,
                background_group=self.background_group,
