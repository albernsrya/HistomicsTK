...
class TestReviewGallery:
    ...
    def test_get_all_rois_from_folder_v2(self):
        ...
        get_all_rois_from_folder_v2(
            gc=cfg.gc,
            folderid=cfg.folderid,
            get_all_rois_kwargs=get_all_rois_kwargs,
            monitor="test",
        )
        ...
        assert set(
            fovdict.keys()) == {"testSlide0ForRevGal", "testSlide1ForRevGal"}

        assert {j.split("_left-")[1]
                for j in fovdict["testSlide0ForRevGal"]} == {
                    "%d_top-%d_bottom-%d_right-%d" % (l, t, b, r)
                    for (l, t, b, r) in [
