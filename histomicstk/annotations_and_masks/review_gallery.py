...
def get_all_rois_from_folder_v2(gc, folderid, get_all_rois_kwargs, monitor=""):
...
def _get_visualization_zoomout(gc, slide_id, bounds, MPP, MAG, zoomout=4):
...
def _plot_rapid_review_vis(
    ...
    rapid_review_vis = _get_review_visualization(rgb=roi_out["rgb"],
                                                 vis=roi_out["visualization"],
...
def create_review_galleries(
    ...
    if upload_results:
        for par in ("gc", "gallery_folderid", "url"):
            if locals()[par] is None:
                raise Exception("%s cannot be None if upload_results!" % par)
    ...
    tile_paths = [
        os.path.join(tilepath_base, j) for j in os.listdir(tilepath_base)
        if j.endswith(".png")
    ]
    tile_paths.sort()
    ...
    def _parse_tilepath(tpath):
        ...
        for attrib in ["id", "left", "top", "bottom", "right"]:
            tileinfo[attrib] = basename.split(attrib + "-")[1].split("_")[0]

        # add URL in histomicsTK
        tileinfo[
            "URL"] = url + "histomicstk#?image=%s&bounds=%s%%2C%s%%2C%s%%2C%s%%2C0" % (
                tileinfo["id"],
                tileinfo["left"],
                tileinfo["top"],
                tileinfo["right"],
    ...
    for galno in range(n_galleries):
        ...
        for row in range(tiles_per_column):
            ...
            for col in range(tiles_per_row):
                ...
                # # get tile from file
                tile = pyvips.Image.new_from_file(tilepath,
                                                  access="sequential")

                # insert tile into mosaic row
                colpos = row_im.width + padding
                row_im = row_im.insert(tile[:3],
                                       colpos,
                                       0,
                                       expand=True,
                                       background=255)
                ...
                if upload_results:
                    ...
                    contours.append({
                        ...
                        "rgb(0,0,0)",
                        "coords_x":
                        ",".join([xmin, xmax, xmax, xmin, xmin]),
                        "coords_y":
                        ",".join([ymin, ymin, ymax, ymax, ymin]),
                    })
        ...
        if upload_results:
            ...
            resps.append(
                gc.uploadFileToFolder(folderId=gallery_folderid,
                                      filepath=savepath,
                                      filename=filename))
            os.remove(savepath)
            ...
            for doc in annotation_docs:
                _ = gc.post("/annotation?itemId=" + resps[-1]["itemId"],
                            json=doc)
        else:
