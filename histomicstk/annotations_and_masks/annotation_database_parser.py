...
def _add_item_to_sqlite(dbcon, item):
    ...
    item_info = {
        k: v
        for k, v in item_info.items() if k in item_info_dtypes.keys()
    }

    # convert to df and add to items table
    item_info_df = DataFrame.from_dict(item_info, orient="index").T
    item_info_df.to_sql(name="items",
                        con=dbcon,
                        if_exists="append",
                        dtype=item_info_dtypes,
                        index=False)
...
def parse_annotations_to_local_tables(
    ...
    annotation_docs, annotation_elements = parse_slide_annotations_into_tables(
        annotations)
