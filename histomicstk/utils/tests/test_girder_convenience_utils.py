...
class TestGirderConvenience:
    ...
    def test_prep(self, girderClient):  # noqa
        ...
        # get original item
        original_iteminfo = cfg.gc.get(
            "/item", parameters={"text": "TCGA-A2-A0YE-01Z-00-DX1"})[0]

        # create a sample folder
        cfg.posted_folder = cfg.gc.post("/folder",
                                        data={
                                            "parentId":
                                            original_iteminfo["folderId"],
                                            "name":
                                            "test"
                                        })
        ...
        cfg.iteminfo = cfg.gc.post(
            "/item/%s/copy" % original_iteminfo["_id"],
            data={
    ...
    def test_get_absolute_girder_path(self):
        ...
        fpath = get_absolute_girder_folderpath(gc=cfg.gc,
                                               folder_info=cfg.posted_folder)
        assert fpath == "Public/test/"

    def test_update_permissions_for_annotations_in_slide(self):
        admininfo = cfg.gc.get("/user", parameters={"text": "admin"})[0]
    ...
    def test_revert_annotations_in_slide(self):
        ...
        _ = cfg.gc.put("/annotation/%s" % anns[0]["_id"],
                       json=anns[0]["annotation"])
