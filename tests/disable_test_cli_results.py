...
class TestCliResults:
    def _runTest(
        ...
        try:
            cmd = [
                arg if not arg.startswith("tmp_") else os.path.join(
                    tmppath, arg) for arg in cli_args
            ]
            cmd += [
                "--%s=%s" %
                (k,
                 v if not v.startswith("tmp_") else os.path.join(tmppath, v))
            ...
            for outpath, options in outputs.items():
                ...
                if "hash" in options:
                    ...
                    if outpath.endswith(".png"):
                        sha.update(
                            PIL.Image.open(outpath).convert(
                                "RGBA").tobytes())  # noqa
                    else:
    ...
    def testListCLI(self):
        ...
        cli_args = (
            os.environ["CLI_LIST_ENTRYPOINT"],
            "--list_cli",
        )
    ...
    def testNucleiDetectionDefaults(self):
        cli_args = (
            ...
            os.path.join(
                TEST_DATA_DIR,
                "TCGA-02-0010-01Z-00-DX4.07de2e55-a8fe-40ee-9e98-bcb78050b9f7-crop.tif",
            ),  # noqa
            "tmp_1.anot",
        )
        ...
        # test with ROI
        cli_kwargs = {"analysis_roi": "0.0, 200.0, 512.0, 512.0"}
        self._runTest(
            cli_args,
            cli_kwargs,
            outputs={
