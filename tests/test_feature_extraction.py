...
def check_fdata_sanity(fdata,
                       expected_feature_list,
                       prefix="",
                       match_feature_count=True):
...
class TestFeatureExtraction:
    ...
    def test_compute_gradient_features(self):
        ...
        fdata = htk_features.compute_gradient_features(cfg.im_nuclei_seg_mask,
                                                       cfg.im_nuclei_stain)
    ...
    def test_compute_morphometry_features(self):
        ...
        if packaging.version.parse(
                skimage.__version__) < packaging.version.parse("0.18"):
            fdata_gtruth = pd.read_csv(
                utilities.getTestFilePath(
                    "Easy1_nuclei_morphometry_features.csv"),
                index_col=None,
            )
            pd.testing.assert_frame_equal(fdata,
    ...
    def test_compute_fsd_features(self):
        ...
        fdata = htk_features.compute_fsd_features(cfg.im_nuclei_seg_mask,
                                                  Fs=Fs)
